...
class IrAttachment(models.Model):
    ...
    def check(self, mode, values=None):
        ...
        if self:
            ...
            self.env['ir.attachment'].flush_model(['res_model', 'res_id', 'create_uid', 'public', 'res_field'])
            self._cr.execute('SELECT res_model, res_id, create_uid, public, res_field, mimetype FROM ir_attachment WHERE id IN %s', [tuple(self.ids)])
            for res_model, res_id, create_uid, public, res_field, mimetype in self._cr.fetchall():
                # Filter out system files (e.g., CSS, JavaScript)
